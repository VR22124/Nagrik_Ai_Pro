...
export default function NextBestAction({ action, ctaLabel, timeEstimate, onStart }) {
  ...
  return (
    <section className="glass-card p-6 md:p-7 flex flex-col gap-4 border-blue-200">
      ...
      <h2 className="text-xl md:text-2xl font-semibold text-slate-900 leading-snug line-clamp-2">
        {action}
      </h2>
      <div className="flex flex-wrap items-center gap-3">
        ...
        {timeEstimate ? (
          <span className="text-sm text-slate-600">Estimated time: {timeEstimate}</span>
        ) : null}
      </div>
    </section>
  );
